      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
          ...
          <p className="text-[11px] text-text3 mb-3">
            For this regime, the bot evaluates strategies in this order until the VIX-capped day cap is reached (same as backtest).
          </p>
          <ol className="space-y-2">
            {(data?.scan_order_sample ?? []).map((s, i) => (
              <li key={s + i} className="flex items-center gap-3 text-sm">
                <span className="w-7 h-7 rounded-lg bg-cyan/10 border border-cyan/20 font-mono font-bold text-cyan flex items-center justify-center text-xs">
                  {i + 1}
                </span>
                <span className="font-semibold text-text1">{s.replace(/_/g, ' ')}</span>
              </li>
            ))}
